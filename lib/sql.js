 ...
 *
 * @returns {Promise.<Array.<{rowCount: number, affectedRowCount: number, totalTimeMS: number}>>} - a promise for the
 *          results object of each query once they're finished, with some extra information attached
 */
Connection.prototype.query = function(batchDef)
{
    ...
    if(batchDef.rollback !== undefined)
    {
        return self.beginTransaction()
            .then(function()
            {
                return Promise.map(batchDef.queries, doQuery)
                    .tap(function()
                    {
