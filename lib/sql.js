...
Connection.prototype.query = function(batchDef)
{
    ...
    function doQuery(queryDef)
    {
        ...
        return new Promise(function(resolve, reject)
        {
            ...
            logger.info("Running query %j of batch %j...", resultSetNum, queryID);
            logger.trace("Query %j#%j: %s", queryID, resultSetNum, logger.dump(queryDef));
            ...
            function onError(error)
            {
                ...
                error.resultSetNum = resultSetNum;
            ...
            function emitFields(fields)
            {
                ...
                {
                    logger.info("Emitting `fields` event with queryID = %s, resultSetNum = %s",
                        logger.dump(queryID), logger.dump(resultSetNum));

                    self.emit('fields', queryID, resultSetNum, fields.map(function(field)
                    {
            ...
            function onRow(row, result)
            {
                ...
                self.emit('row', queryID, resultSetNum, row);
            } // end onRow
